...
#include <mutex>
#include <custom_msg2/custom_msg.h>
    ...
    { 
	  if(mbGetNewCamPose)
	  {
	      ...
	      // Publicar descriptores
					...
					header.seq = 9999; // user defined counter
					header.stamp = ros::Time::now(); // time
					img_bridge = cv_bridge::CvImage(header, sensor_msgs::image_encodings::RGB8, this->mpTracker->mCurrentFrame.mDescriptors);
					img_bridge.toImageMsg(img_msg); // from cv_bridge to sensor_msgs::Image
					Descriptors_pub_.publish(img_msg);
	      		
		  //Publicar array de keypoints
					
				std::vector<cv::KeyPoint> mvKeys = this->mpTracker->mCurrentFrame.mvKeys;
				custom_msg2::custom_msg arrayKP;
					

					//Test 1
					if(mvKeys.size()>0){

							for(unsigned i=0; i < mvKeys.size(); i++ ){

													cv::KeyPoint currentKP = mvKeys.at(i);
													
													//asignando x,y
													custom_msg2::Point2f pointmsg;
													pointmsg.x = currentKP.pt.x;
													pointmsg.y = currentKP.pt.y;

													//asignando otros valores de la clase
													custom_msg2::KeyPoint kpmsg;
													kpmsg.pt = pointmsg;
													kpmsg.size = currentKP.size;
													kpmsg.angle = currentKP.angle;
													kpmsg.response = currentKP.response;
													kpmsg.octave = currentKP.octave;
													kpmsg.class_id = currentKP.class_id;
													kpmsg.idKeypoint = i+1;

													//agregando al array
													arrayKP.points.push_back(kpmsg);
													
												}
												printf("%d\n",arrayKP.points.size() );
												KeyPoints_pub_.publish(arrayKP);
					}
			...
			// Publicar trayectoria
					...
					/*if(this->arrayTj){
						if(this->arrayTj.header){
							std_msgs::Header header;
							header.seq = 9999; // user defined counter
							header.stamp = ros::Time::now(); // time
							header.frame_id = "Trajectory";

							this->arrayTj.header = header;
						}*/

					    std_msgs::Header header2;
							header2.seq = 9999; // user defined counter
							header2.stamp = ros::Time::now(); // time
							header2.frame_id = "ground";

							this->arrayTj.header = header2;
						this->arrayTj.poses.push_back(camPose2Ground.pose);
						Trajectory_pub_.publish(this->arrayTj);
					//}
...
void SlamDataPub::Run()
{
    ...
    Descriptors_pub_ = nh.advertise<sensor_msgs::Image>("Descriptors", 1);
    KeyPoints_pub_ = nh.advertise<custom_msg2::custom_msg>("KeyPoints",1);

    Trajectory_pub_ = nh.advertise<geometry_msgs::PoseArray>("Trajectory",1);
